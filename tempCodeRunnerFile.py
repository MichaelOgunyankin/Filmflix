...
def report():
    ...
    # Search by Title or Year Released or Rating or Duration or Genreif search_field.title() == "Year Released":
        ...
        # Handle year released specifically (assuming year is an integer)try:
            ...
            str_input = int(input(f"Enter the year released: "))  # Prompt for integer year            search_query = f"SELECT * FROM tblFilms WHERE yearReleased = ?"  # Use exact match            dbCursor.execute(search_query, (str_input,))        except ValueError:
